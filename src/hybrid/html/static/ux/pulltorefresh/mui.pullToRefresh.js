(function($, window, document) {
  let STATE_BEFORECHANGEOFFSET = "beforeChangeOffset";
  let STATE_AFTERCHANGEOFFSET = "afterChangeOffset";

  let EVENT_PULLSTART = "pullstart";
  let EVENT_PULLING = "pulling";
  let EVENT_BEFORECHANGEOFFSET = STATE_BEFORECHANGEOFFSET;
  let EVENT_AFTERCHANGEOFFSET = STATE_AFTERCHANGEOFFSET;
  let EVENT_DRAGENDAFTERCHANGEOFFSET = "dragEndAfterChangeOffset";

  let CLASS_TRANSITIONING = $.className("transitioning");
  let CLASS_PULL_TOP_TIPS = $.className("pull-top-tips");
  let CLASS_PULL_BOTTOM_TIPS = $.className("pull-bottom-tips");
  let CLASS_PULL_LOADING = $.className("pull-loading");
  let CLASS_SCROLL = $.className("scroll");

  let CLASS_PULL_TOP_ARROW =
    $.className("pull-loading") +
    " " +
    $.className("icon") +
    " " +
    $.className("icon-pulldown");
  let CLASS_PULL_TOP_ARROW_REVERSE =
    CLASS_PULL_TOP_ARROW + " " + $.className("reverse");
  let CLASS_PULL_TOP_SPINNER =
    $.className("pull-loading") + " " + $.className("spinner");
  let CLASS_HIDDEN = $.className("hidden");

  let SELECTOR_PULL_LOADING = "." + CLASS_PULL_LOADING;
  $.PullToRefresh = $.Class.extend({
      ...
      switch (e.type) {
        case $.EVENT_START:
          this.isInScroll &&
            ...
            e.preventDefault();
          //只有滚动条在顶部，才可以下拉刷新------linzhichao  2016-9-23
          this.enablePull = document.body.scrollTop == 0;
          break;
        case "drag":
          this._drag(e);
          break;
      ...
      if ($.isFunction(self.options.down.callback)) {
        self.pullDownTips = (function() {
          ...
          if (!element) {
            ...
            element.innerHTML =
              '<div class="mui-pull-top-wrapper"><span class="mui-pull-loading mui-icon mui-icon-pulldown"></span></div>';
    ...
    initPullUpTips: function() {
      ...
      if ($.isFunction(self.options.up.callback)) {
        self.pullUpTips = (function() {
          let element = self.element.querySelector(
          ...
          if (!element) {
            ...
            element.innerHTML =
              '<div class="mui-pull-bottom-wrapper"><span class="mui-pull-loading">' +
    ...
    _canPullUp: function() {
      ...
      return (
        window.pageYOffset + window.innerHeight + this.options.up.offset >=
    ...
    _drag: function(e) {
      ...
      if (this.isDragging) {
        ...
        let state =
          deltaY > this.options.down.height
            ? STATE_AFTERCHANGEOFFSET
            : STATE_BEFORECHANGEOFFSET;
        if (this.state !== state) {
          this.state = state;
    ...
    removePullDownTips: function() {
      if (this.pullDownTips) {
        try {
          this.pullDownTips.parentNode &&
            this.pullDownTips.parentNode.removeChild(this.pullDownTips);
    ...
    pulling: function(deltaY) {
      this.pullDownTips.style.webkitTransform =
    ...
    pullDownLoading: function() {
      ...
      this.pullDownTips.style.webkitTransform =
        "translate3d(0," + this.options.down.height + "px,0)";
    ...
    setStopped: function(stopped) {
      if (stopped != this.stopped) {
        ...
        this.pullUpTips &&
          this.pullUpTips.classList[stopped ? "add" : "remove"](CLASS_HIDDEN);
      }
    },
