(function($) {
  ...
  $.PullToRefresh = $.PullToRefresh.extend({
    init: function(element, options) {
      ...
      this.options.down.tips.colors = this.options.down.tips.colors.map(
        function(color) {
          return {
      ...
      if ($.isFunction(self.options.down.callback)) {
        self.pullDownTips = (function() {
          ...
          if (!element) {
            ...
            element.innerHTML =
              '<div class="mui-pull-top-wrapper"><div class="mui-pull-top-canvas"><canvas id="pullDownTips" width="' +
              self.options.down.tips.size +
              '" height="' +
              self.options.down.tips.size +
              '"></canvas></div></div>';
    ...
    pulling: function(deltaY) {
      ...
      let ratioPI = Math.min(1, ratio * 2);
      this.pullDownTips.style.webkitTransform =
        "translate3d(0," + (deltaY < 0 ? 0 : deltaY) + "px,0)";
      this.pullDownCanvas.style.opacity = ratioPI;
      this.pullDownCanvas.style.webkitTransform =
        "rotate(" + 300 * ratio + "deg)";
      let canvas = this.pullDownCanvas;
      ...
      this.canvasUtils.drawArcedArrow(
        ctx,
        size / 2 + 0.5,
        size / 2,
        size / 4,
        ...
        0.7853981633974483,
        25,
        this.options.down.tips.lineWidth,
        this.options.down.tips.lineWidth
      );
    },
    ...
    canvasUtils: (function() {
      ...
      let drawHead = function(ctx, x0, y0, x1, y1, x2, y2, style) {
        "use strict";
      ...
      let drawArcedArrow = function(
        ...
        if (which & 2) {
          ...
          drawArrow(
            ctx,
            sx - lineRatio * Math.sin(endangle),
            sy + lineRatio * Math.cos(endangle),
            destx - lineRatio * Math.sin(endangle),
            desty + lineRatio * Math.cos(endangle),
            style,
            2,
            angle,
            d
      ...
      let spinColors = function(currentIteration, totalIterations) {
        let step = currentIteration % totalIterations;
        if (step < oldStep) colors.push(colors.shift());
        let c0 = colors[0],
          c1 = colors[1],
          r = minmax(
            easeLinear(step, c0.r, c1.r - c0.r, totalIterations),
            c0.r,
            c1.r
          ),
          g = minmax(
            easeLinear(step, c0.g, c1.g - c0.g, totalIterations),
            c0.g,
            c1.g
          ),
          b = minmax(
            easeLinear(step, c0.b, c1.b - c0.b, totalIterations),
            c0.b,
            c1.b
          );
      ...
      var spin = function(t) {
        let timeCurrent = t || new Date().getTime();
        if (!startTime) {
          startTime = timeCurrent;
        }
        tick = timeCurrent - startTime;
        acc = easeInOutQuad(
          (tick + tail_duration / 2) % tail_duration,
          0,
          duration,
          tail_duration
        );
        ...
        ctx.arc(
          ...
          size / 2,
          size / 4,
          (parseInt(head - tail) % 360) * rad,
          (parseInt(head) % 360) * rad,
          false
        );
        ctx.stroke();
      ...
      let init = function(canvas, options) {
        canvasObj = canvas;
