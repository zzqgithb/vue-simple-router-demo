(function($, window) {
  ...
  $.Zoom = function(element, options) {
    ...
    zoom.scale = function(style, scale) {
      scale = scale || 1;
      //图片旋转--linzhichao
      let rotate = zoom.zoomer.getAttribute("data-rotate") || "0";
      style["webkitTransform"] =
        "translate3d(0,0,0) scale(" + scale + ") rotate(" + rotate + "deg)";
      return zoom;
    };
    ...
    zoom.onPinchend = function(e) {
      scale = Math.max(
        Math.min(scale, zoom.options.maxZoom),
        zoom.options.minZoom
      );
    ...
    zoom.onTouchMove = function(e) {
      ...
      if (!imageIsMoved) {
        ...
        imageWidth = zoom.zoomer.offsetWidth;
        imageHeight = zoom.zoomer.offsetHeight;
        let translate = $.parseTranslateMatrix(
          $.getStyles(zoom.scroller, "webkitTransform")
        );
    ...
    zoom.onTouchEnd = function(e) {
      ...
      zoom
        .scrollerTransition(momentumDuration)
        .scrollerTransform(imageCurrentX, imageCurrentY);
    };
