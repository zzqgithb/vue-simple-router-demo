...
(function (EXP_TYPE) {
    EXP_TYPE["in"] = "in";
    EXP_TYPE["inSelect"] = "inSelect";
    EXP_TYPE["notIn"] = "notIn";
    EXP_TYPE["notInSelect"] = "notInSelect";
    EXP_TYPE["notNull"] = "noNull";
    EXP_TYPE["isNull"] = "isNull";
    EXP_TYPE["between"] = "between";
    EXP_TYPE["ne"] = "!=";
    EXP_TYPE["gt"] = ">";
    EXP_TYPE["ge"] = ">=";
    EXP_TYPE["lt"] = "<";
    EXP_TYPE["le"] = "<=";
    EXP_TYPE["likeLeft"] = "%like";
    EXP_TYPE["likeRight"] = "like%";
    EXP_TYPE["like"] = "like";
    EXP_TYPE["eq"] = "=";
    EXP_TYPE["and"] = "and";
    EXP_TYPE["or"] = "or";
...
export var FORM_DATA_TYPE;
(function (FORM_DATA_TYPE) {
    FORM_DATA_TYPE["select"] = "select";
    FORM_DATA_TYPE["child"] = "child";
 ...
 * @constructor
 */
function FormDataSql(_a) {
    var { type, fieldId, mainFuncId, childFuncId } = _a, fields = __rest(_a, ["type", "fieldId", "mainFuncId", "childFuncId"]);
    this.__type = type;
...
const safetySqlUtil = {
     ...
     */
    buildSqls({ tree, quick, group, order, mark, column, custom, formData, strategyId, strategy, replace, coverOrder, coverWhere, } = {}) {
        return {
    ...
    buildCustomSql({ code, type, cn, value, _value }) {
        ...
        if ([EXP_TYPE.in, EXP_TYPE.notIn].includes(type) && !Array.isArray(value)) {
            console.error(`type为【${EXP_TYPE.in},${EXP_TYPE.notIn}】时value类型为数组`);
            return;
        }
        if ([EXP_TYPE.inSelect, EXP_TYPE.notInSelect].includes(type) &&
            !(value instanceof ChildCustomSql)) {
            console.error(`type为【${EXP_TYPE.inSelect},${EXP_TYPE.notInSelect}】时value类型为ChildCustomSql`);
            return;
        }
    ...
    buildFormData(_a) {
        ...
        const params = Object.assign({ type,
            fieldId }, fields);
...
//# sourceMappingURL=safetySqlUtil.js.map